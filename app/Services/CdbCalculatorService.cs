...
{
    public ResultCdb CalcularCdb(int qtdMeses, double ValorInicial)
    {
        if (ValorInicial <= 0)
        {
        ...
        var valorImposto = CalcularCdbImposto(qtdMeses, lucro);

        double valorResultadoLiquido = ValorInicial + (lucro - valorImposto);

        var result = new ResultCdb { ValorBruto = resultados[resultados.Count - 1], ValorLiquido = valorResultadoLiquido };

        return result;
    }

    public double CalcularCdbImposto(int qtdMeses, double valorResultadoBruto)
    {
